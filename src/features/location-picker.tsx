...
const fetcher = async (url: string) => {
  try {
    ...
    const locationResponse = await axios.get(`https://ipinfo.io/${ipData.ip}?token=${process.env.PLASMO_PUBLIC_LOCATION}`);
...
export const LocationPicker = ({ setLoading }) => {
  ...
  return (
    <div style={{
      ...
      Your country is {countryList.getName(locationData.country)} and city is {locationData.city}
    </div>
  );
};
